...
var Terminal = (function () {
	...
	promptInput = function (terminalObj, message, PROMPT_TYPE, callback) {
		...
		var startChars = '> ';
		var startCharsLen = startChars.length;

		terminalObj._inputLine.textContent = startChars + ''
			...
			if (PROMPT_TYPE === PROMPT_CONFIRM || e.which === 13) {
				...
				if (typeof(callback) === 'function') {
					if (PROMPT_TYPE === PROMPT_CONFIRM) {
						callback(inputValue.slice(startCharsLen).toUpperCase()[0] === 'Y' ? true : false)
					} else callback(inputValue.slice(startCharsLen))
				}
			}
		}
		if (firstPrompt) {
	...
	var TerminalConstructor = function (id) {
		...
		this.print = function (message, color=this.html.style.color) {
